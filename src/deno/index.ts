...
import type { LoaderFunctionArgs, ActionFunctionArgs } from "@remix-run/deno";
...
export function RemixAuth(config: RemixAuthConfig) {
    return {
        loader: (args: LoaderFunctionArgs | ActionFunctionArgs) => {
            return loader(config, Deno.env.toObject())(args);
        },
        action: (args: LoaderFunctionArgs | ActionFunctionArgs) => {
            return action(config, Deno.env.toObject())(args);
        },
        getSession: (...args: Parameters<ReturnType<typeof getCsrfToken>>) => {
            return getSession(config, Deno.env.toObject())(...args);
        },
        getCsrfToken: (
            ...args: Parameters<ReturnType<typeof getCsrfToken>>
        ) => {
            return getCsrfToken(config, Deno.env.toObject())(...args);
        },
        signIn: (...args: Parameters<ReturnType<typeof signIn>>) => {
            return signIn(config, Deno.env.toObject())(...args);
        },
        signOut: (...args: Parameters<ReturnType<typeof signOut>>) => {
            return signOut(config, Deno.env.toObject())(...args);
        },
    };
}
